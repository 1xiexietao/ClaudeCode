...
**触发条件**（同时满足）：
1. 订单创建时间 >= X天（domestic_wait_days）
2. 订单内商品在易仓有可用库存
...
定时任务扫描:
  ...
  IF 当前时间 - 订单创建时间 >= domestic_wait_days:
    调用易仓库存接口查询该订单商品的可用库存
    IF 所有商品可用库存 >= 订单数量:
      标记为"可发货"
    ELSE IF 部分商品有库存:
      调用Shopify fulfillmentOrderSplit 二次拆分
      有库存的商品 → 拆出先发货
      无库存的商品 → 保持等待
    ELSE:
      保持等待，下次再检查
...
**流程说明**：拆单完成后，国内仓订单自动同步易仓；Agent定时监听易仓发货状态，获取物流单号并回写Shopify。

**触发方式**：
- 同步易仓：拆单完成后自动触发（国内仓订单）
- 发货状态回传：Agent定时任务（每10分钟）
...
└──────────────────────────────────────────────────────────────────────────────┘
  ...
  【阶段一：同步易仓】拆单完成后自动触发
  ─────────────────────────────────────────────────────────────────────────────
    Step 1: Agent获取拆单后的国内仓子订单
      ↓
    Step 2: Agent调用易仓syncOrder接口，同步订单信息+商品明细
      ↓
    Step 3: 更新子订单 wms_sync_status = SYNCED，记录 wms_order_no
      ↓
    Step 4: 易仓创建订单，进入 入库→出库→发货 流程


  【阶段二：发货状态回传】Agent定时任务（每10分钟）
  ─────────────────────────────────────────────────────────────────────────────
    Step 5: Agent调用易仓getOrderList接口，筛选 status=已发货
      ↓
    Step 6: Agent通过 order_no 匹配履约平台中的子订单
      ↓
    Step 7: Agent提取 tracking_no（物流单号）和 tracking_company（物流公司）
      ↓
    Step 8: Agent更新履约平台子订单
            - status → SHIPPED
            - tracking_number = 物流单号
            - tracking_company = 物流公司
      ↓
    Step 9: Agent调用Shopify fulfillmentCreate 回写物流信息
      ↓
    Step 10: Shopify订单标记已发货，买家可查看物流信息
```
...
│    ↓                                                      │
│  Step 3: Agent调用易仓库存接口，查询订单商品的可用库存           │
│    ↓                                                      │
│  Step 4: Agent判断拆分                                       │
│    - 所有商品可用库存 >= 订单数量 → 标记"可发货"，同步易仓出库  │
│    - 部分商品有库存 → 调用Shopify API二次拆分                 │
│      有库存商品 → 拆出先发货                                 │
│      无库存商品 → 保持等待                                   │
│    - 所有商品无库存 → 跳过，下次再检查                         │
│    ↓                                                      │
│  Step 5: Agent二次拆分后，有库存部分同步易仓出库发货             │
└──────────────────────────────────────────────────────────┘
```
